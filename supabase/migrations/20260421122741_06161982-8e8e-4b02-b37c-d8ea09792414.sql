...
BEGIN
  ...
  LOOP
    ...
    IF _img_url !~* '^https?://.+' THEN
      RAISE EXCEPTION 'Carousel card %: image_url must start with http:// or https://', _key
        USING ERRCODE = 'check_violation';
    END IF;
  END LOOP;
...
DROP TRIGGER IF EXISTS validate_campaign_carousel_mapping_trigger ON public.campaigns;

CREATE TRIGGER validate_campaign_carousel_mapping_trigger
BEFORE INSERT OR UPDATE ON public.campaigns
FOR EACH ROW
EXECUTE FUNCTION public.validate_campaign_carousel_mapping();
